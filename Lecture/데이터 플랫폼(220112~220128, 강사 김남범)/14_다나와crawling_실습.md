...
- 내장형 SSD를 검색해 읽기와 쓰기, 가격에 대한 산점도 그리기
...
- 데이터 전처리
    ...
    for name in data['name']:
        if 'Western Digital' in name:
            name_info = name.split(' ',2)
    ...
    for spec in data['spec']:
        ...
        system_name = spec_list[2]
        system_list.append(system_name)    
    
        
        for spec_info in spec_list:
            if '순차읽기' in spec_info:
                read = spec_info.split(': ')[1]
            elif '순차쓰기' in spec_info:
                write = spec_info.split(': ')[1]   
        write_list.append(write)
        read_list.append(read)
    
    # 필요없는 단위 제거 및 정수로 타입 변경
    new_system_list = []
    for system in system_list:
        system_new = system.split(' (')[0]
        new_system_list.append(system_new)
        
    new_write_list = []
    for write in write_list:
        try:
            write_new = int(write.split('M')[0].replace(',',''))
            new_write_list.append(write_new)
        except:
            new_write_list.append(None)
    
    new_read_list = []
    for read in read_list:
        try:
            read_new = int(read.split('M')[0].replace(',',''))
            new_read_list.append(read_new)
        except:
            new_read_list.append(None)
    
    new_price_list = []
    for price in data['price']:
        try:
            price_new = int(price.split('원')[0].replace(',',''))
            new_price_list.append(price_new)
        except:
    ...
    pd_data['방식'] = new_system_list
    pd_data['순차쓰기(MB/s)'] = new_write_list
    pd_data['순차읽기(MB/s)'] = new_read_list
    pd_data['가격(원/1GB)'] = new_price_list
    pd_data.to_excel('./sdd_final.xlsx', index=False)
